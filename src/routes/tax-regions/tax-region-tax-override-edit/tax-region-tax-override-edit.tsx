...
const useDefaultRulesValues = (
  ...
  if (!taxRate) {
    return {
      isPending: true,
      initialValues: { product: [], product_type: [], shipping_option: [] }
    };
  }
  ...
  if (productsResult.enabled) {
    ...
    if (result.products) {
      const productMap = new Map(result.products.map(product => [product.id, product.title]));
      initialRulesValues.product = idsByReferenceType[TaxRateRuleReferenceType.PRODUCT].map(id => ({
        value: id,
  ...
  if (productTypesResult.enabled) {
    ...
    if (result.product_types) {
      const productTypeMap = new Map(result.product_types.map(type => [type.id, type.value]));
      initialRulesValues.product_type = idsByReferenceType[
        TaxRateRuleReferenceType.PRODUCT_TYPE
      ].map(id => ({
        value: id,
  ...
  if (shippingOptionsResult.enabled) {
    ...
    if (result.shipping_options) {
      ...
      initialRulesValues.shipping_option = idsByReferenceType[
        TaxRateRuleReferenceType.SHIPPING_OPTION
      ].map(id => ({
        value: id,

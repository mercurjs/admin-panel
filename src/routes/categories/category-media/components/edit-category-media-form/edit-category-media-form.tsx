...
import { Button, Checkbox, clx, CommandBar, Tooltip, toast } from '@medusajs/ui';
...
import { EditCategoryMediaSchemaType } from '../../types';
...
export const EditCategoryMediaForm = ({ category, type }: EditCategoryMediaFormProps) => {
  ...
  const { mutateAsync: updateProductCategoryDetails, isPending } =
    useUpdateProductCategoryDetails();

  const thumbnailId = category.category_detail?.thumbnail_id ?? null;
  const bannerId = category.category_detail?.banner_id ?? null;
  const iconId = category.category_detail?.icon_id ?? null;
  ...
  const iconMedia = useMemo(() => {
    ...
    return category.category_detail?.media?.find(m => m.id === iconId);
  }, [category.category_detail?.media, iconId]);

  const defaultMedia: EditCategoryMediaSchemaType['media'] = useMemo(() => {
    if (isIconType) {
      return iconMedia
        ? [
            {
    ...
    return (category.category_detail?.media ?? [])
  ...
  }, [bannerId, category.category_detail?.media, iconId, iconMedia, isIconType, thumbnailId]);
  ...
  const handleSubmit = form.handleSubmit(async ({ media }) => {
    const filesToUpload = !isIconType
      ? media.map((m, i) => ({ file: m.file, index: i })).filter(m => !!m.file)
      : [
          {
    ...
    if (isIconType) {
      ...
      await updateProductCategoryDetails(
        {
          id: category.id,
          payload
        },
        {
          onSuccess: () => {
            ...
            handleSuccess(`/categories/${category.id}`);
          },
          onError: (error: any) => {
  ...
  return (
    <RouteFocusModal.Form
      blockSearchParams
      form={form}
      data-testid="collection-edit-media-form"
    >
      <KeyboundForm
      ...
      >
        ...
        <RouteFocusModal.Body
          ...
          data-testid="collection-edit-media-form-body"
        >
                    ...
                    {activeId ? (
                      <MediaGridItemOverlay
                        media={{
                          ...
                          isBanner:
                            !!bannerId && fields.find(m => m.field_id === activeId)?.id === bannerId
                        }}

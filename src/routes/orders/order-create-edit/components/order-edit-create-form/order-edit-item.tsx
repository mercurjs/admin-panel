...
function OrderEditItem({ item, currencyCode, orderId }: OrderEditItemProps) {
  ...
  const isAddedItem = useMemo(() => !!item.actions?.find(a => a.action === 'ITEM_ADD'), [item]);
  ...
  return (
    ...
    >
      <div
        className="flex flex-col items-center gap-x-2 gap-y-2 p-3 text-sm md:flex-row"
        data-testid={`order-edit-item-${item.id}-content`}
      >
        <div
          className="flex flex-1 items-center justify-between"
          data-testid={`order-edit-item-${item.id}-info`}
        >
          <div
            className="flex flex-row items-center gap-x-3"
            data-testid={`order-edit-item-${item.id}-details`}
          >
            <Thumbnail
              src={item.thumbnail}
              data-testid={`order-edit-item-${item.id}-thumbnail`}
            />

            <div
              className="flex flex-col"
              data-testid={`order-edit-item-${item.id}-text`}
            >
              <div data-testid={`order-edit-item-${item.id}-title`}>
                ...
                {item.variant_sku && (
                  <span data-testid={`order-edit-item-${item.id}-sku`}>({item.variant_sku})</span>
                )}
              </div>
              <Text
                as="div"
                className="txt-small text-ui-fg-subtle"
                data-testid={`order-edit-item-${item.id}-subtitle`}
              >
                {item.subtitle}
              </Text>
            </div>
          ...
          {isAddedItem && (
            <Badge
              ...
              color="blue"
              className="mr-1"
              data-testid={`order-edit-item-${item.id}-new-badge`}
            >
              {t('general.new')}
            </Badge>
          )}
          ...
          {isItemRemoved ? (
            <Badge
              ...
              color="red"
              className="mr-1"
              data-testid={`order-edit-item-${item.id}-removed-badge`}
            >
              {t('general.removed')}
            </Badge>
          ) : (
        ...
        <div
          ...
          data-testid={`order-edit-item-${item.id}-actions`}
        >
          <div
            className="flex flex-grow items-center gap-2"
            data-testid={`order-edit-item-${item.id}-quantity`}
          >
            <Input
            ...
            <Text
              className="txt-small text-ui-fg-subtle"
              data-testid={`order-edit-item-${item.id}-quantity-label`}
            >
              {t('fields.qty')}
            </Text>
          </div>
          ...
          <div
            className="txt-small mr-2 flex flex-shrink-0 text-ui-fg-subtle"
            data-testid={`order-edit-item-${item.id}-total`}
          >
            <MoneyAmountCell

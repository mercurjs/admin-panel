...
import { useSelectableRowSelection } from '../../../hooks/use-selectable-row-selection';
import { useAdminManagedLocations } from '../../../order-detail/context/admin-managed-locations-context';
...
export const AddExchangeInboundItemsTable = ({
  ...
  const getRowDisabledReason = useMemo(() => {
    return (item: AdminOrderLineItem) => {
      ...
      if (!canAdminActOnItem(item)) return t('orders.exchanges.cantCreateExchangeByAdmin');
  ...
  const columns = useExchangeItemTableColumns(currencyCode, getRowDisabledReason);

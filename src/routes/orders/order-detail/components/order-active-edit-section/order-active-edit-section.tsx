...
import { useCancelOrderEdit, useConfirmOrderEdit } from '@hooks/api/order-edits';
...
function EditItem({ item, quantity }: { item: HttpTypes.AdminOrderLineItem; quantity: number }) {
  return (
    <div
      key={item.id}
      className="items-center gap-x-2 text-ui-fg-subtle"
    >
      <div className="flex items-center gap-x-2">
        ...
        <span className="txt-small font-medium text-ui-fg-subtle">{item.title}</span>
...
export const OrderActiveEditSection = ({ order }: OrderActiveEditSectionProps) => {
      ...
      <Container className="flex items-center justify-between p-0">
        <div className="flex w-full flex-col divide-y divide-dashed">
          ...
          >
            ...
            <Heading
              level="h2"
              data-testid="order-active-edit-heading"
            >
              {t(isPending ? 'orders.edits.panel.titlePending' : 'orders.edits.panel.title')}
            </Heading>
          </div>

          {/*ADDED ITEMS*/}
          {!!addedItems.length && (
            <div className="txt-small flex flex-row px-6 py-4 text-ui-fg-subtle">

...
export function RequestSellerDetail({ request, open, close }: Props) {
  ...
  return (
    <Drawer
      open={open}
      onOpenChange={close}
      data-testid={`request-seller-detail-${request.id}`}
    >
      ...
      <Drawer.Content data-testid={`request-seller-detail-${request.id}-content`}>
        <Drawer.Header data-testid={`request-seller-detail-${request.id}-header`}>
          <Drawer.Title data-testid={`request-seller-detail-${request.id}-title`}>
            Review seller request
          </Drawer.Title>
        </Drawer.Header>
        <Drawer.Body
          className="p-4"
          data-testid={`request-seller-detail-${request.id}-body`}
        >
          <fieldset data-testid={`request-seller-detail-${request.id}-seller-name-fieldset`}>
            <legend
              className="mb-2"
              data-testid={`request-seller-detail-${request.id}-seller-name-legend`}
            >
              Seller name
            </legend>
            <Container data-testid={`request-seller-detail-${request.id}-seller-name-container`}>
              <Text data-testid={`request-seller-detail-${request.id}-seller-name-value`}>
          ...
          <fieldset
            className="mt-2"
            data-testid={`request-seller-detail-${request.id}-member-fieldset`}
          >
            <legend
              className="mb-2"
              data-testid={`request-seller-detail-${request.id}-member-legend`}
            >
              Member
            </legend>
            <Container data-testid={`request-seller-detail-${request.id}-member-container`}>
              <Text data-testid={`request-seller-detail-${request.id}-member-value`}>
          ...
          <fieldset
            className="mt-2"
            data-testid={`request-seller-detail-${request.id}-email-fieldset`}
          >
            <legend
              className="mb-2"
              data-testid={`request-seller-detail-${request.id}-email-legend`}
            >
              Email
            </legend>
            <Container data-testid={`request-seller-detail-${request.id}-email-container`}>
              <Text data-testid={`request-seller-detail-${request.id}-email-value`}>
                {requestData?.provider_identity_id ?? 'N/A'}
              </Text>
            </Container>
          </fieldset>
          <Container
            className="mt-4"
            data-testid={`request-seller-detail-${request.id}-request-information`}
          >
            <div
              className="flex items-center gap-2"
              data-testid={`request-seller-detail-${request.id}-request-information-header`}
            >
              <InformationCircle />
              <Text
                className="font-semibold"
                data-testid={`request-seller-detail-${request.id}-request-information-title`}
              >
                Request information
              </Text>
            </div>
            <Text
              data-testid={`request-seller-detail-${request.id}-submitted-on`}
            >{`Submitted on ${formatDate(request.created_at)}`}</Text>
            {request.reviewer_id && (
              <Text
                data-testid={`request-seller-detail-${request.id}-reviewed-on`}
              >{`Reviewed on ${formatDate(request.updated_at)}`}</Text>
            )}
            {request.reviewer_note && (
              <Text
                data-testid={`request-seller-detail-${request.id}-reviewer-note`}
          ...
          {request.status === 'pending' && (
            <>
              ...
              <Button
                variant="secondary"
                onClick={close}
                data-testid={`request-seller-detail-${request.id}-cancel-button`}
              >
                Cancel
              </Button>
            </>

...
import type { AdminRefundReasonDeleteResponse, HttpTypes } from '@medusajs/types';
import {
  useMutation,
  useQuery,
...
export const useRefundReasons = (
  ...
  options?: Omit<
    UseQueryOptions<AdminRefundReasonListResponse, FetchError, AdminRefundReasonListResponse>,
    'queryFn' | 'queryKey'
  >
) => {
  const { data, ...rest } = useQuery({
    queryFn: () => sdk.admin.refundReason.list(query) as Promise<AdminRefundReasonListResponse>,
...
export const useRefundReason = (
  ...
  options?: Omit<
    UseQueryOptions<AdminRefundReasonResponse, FetchError, AdminRefundReasonResponse>,
    'queryFn' | 'queryKey'
  >
) => {
  const { data, ...rest } = useQuery({
    queryFn: () =>
      sdk.client.fetch<AdminRefundReasonResponse>(`/admin/refund-reasons/${id}`, {
        query
      }),
...
export const useUpdateRefundReason = (
  id: string,
  options?: UseMutationOptions<AdminRefundReasonResponse, FetchError, AdminUpdateRefundReason>
) => {
  return useMutation({
    mutationFn: async data =>
      sdk.client.fetch<AdminRefundReasonResponse>(`/admin/refund-reasons/${id}`, {
...
export const useDeleteRefundReasonLazy = (
  ...
  return useMutation({
    mutationFn: (id: string) =>
      sdk.client.fetch<AdminRefundReasonDeleteResponse>(`/admin/refund-reasons/${id}`, {
        method: 'DELETE'
